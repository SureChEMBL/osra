  ...
  if( !hscan_valid )
    ...
    for( int col = b.left(); col <= b.right(); ++col )
      ...
      for( int row = b.top(); row <= b.bottom(); ++row )
        ...
        switch( state )
          {
          case 0: if( black ) state = 1; else break;
...
int Features::test_misc( const Rectangle & charbox ) const throw()
  {
  const Block & b = *_block;
  ...
  if( tp.minima( b.height() / 4 ) == 1 && bp.minima( b.height() / 4 ) == 1 )
    {
    if( b.height() > 2 * b.width() && rp.increasing( 1 ) &&
