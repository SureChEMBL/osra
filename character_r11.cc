...
//
void Character::recognize1( const Charset & charset, const Rectangle & charbox ) throw()
  {
  if( blocks() == 1 ) recognize11( charset, charbox );
  else if( blocks() == 2 ) recognize12( charset, charbox );
  else if( blocks() == 3 ) recognize13( charset, charbox );
  }
...
// #$&'()*+,-./<>@[\]^_`{|}~¬
//
void Character::recognize11( const Charset & charset, const Rectangle & charbox ) throw()
  {
...
// '()*+,-./<>@[\]^_`{|}~¬
//
void Character::recognize110( const Charset & charset, const Rectangle & charbox ) throw()
  {
  const Block & b = block( 0 );
  Features f( b );
  ...
  if( code == 'N' && b.width() > b.height() && b.top() >= charbox.top() &&
    ...
    if( col > f.hp.pos( 40 ) && col < f.hp.pos( 50 ) )
      {
      only_guess( 0, b.left() ); add_guess ( 'r', b.left() + col );
  ...
  if( f.bp.minima() <= 2 &&
    ...
    if( code == 'n' )	// Looks for '"' or merged 'rr' or 'TT' or 'rt' or 'fl'
      {
      ...
      if( b.width() > b.height() && 10 * f.lp[f.lp.pos(10)] < b.width() &&
        ...
        if( 10 * rgap > b.width() && !b.escape_top( b.vcenter(), b.right() ) )
          {
          only_guess( 0, b.left() );
          const int lgap = f.lp[f.lp.pos(50)];
          if( 10 * lgap > b.width() && !b.escape_top( b.vcenter(), b.left() ) &&
              Ocrad::similar( lgap, rgap, 10 ) && b.top() < charbox.top() )
            { add_guess( 'T', b.hcenter() ); add_guess( 'T', b.right() ); }
          else { add_guess( 'r', b.hcenter() ); add_guess( 'r', b.right() ); }
          return;
          }
        }
      if( 2 * f.lp[f.lp.pos(10)] > b.width() && !f.rp.increasing( f.rp.pos( 75 ) ) )
        ...
        if( col <= b.hpos( 95 ) && !b.escape_top( b.vcenter(), col ) )
          {
          only_guess( 0, b.left() );
          add_guess( 'r', b.hcenter() ); add_guess( 't', b.right() );
          return;
          }
        }
      ...
      if( f.rp.isctip( 33 ) )
        {
        only_guess( 0, b.left() ); add_guess( 'f', b.hcenter() );
        add_guess( 'i', b.right() ); return;
        }
      }
  ...
  if( f.tp.minima( b.height() / 4 ) == 3 )
    ...
    if( !b.bottom_hook( &hdiff ) &&
        ( f.vscan()[b.vcenter() - b.top()] < 4 || !b.escape_top( b.vcenter(), b.hcenter() ) ) )
      add_guess( 'w', 0 );
    return;
    }
...
// 0469ADOPQRabdegopq#
//
void Character::recognize111( const Charset & charset, const Rectangle & charbox ) throw()
  {
  const Block & b = block( 0 );
...
// 8BQg$&
//
void Character::recognize112( const Rectangle & charbox ) throw()
  {
  const Block & b = block( 0 );
  ...
  if( Ocrad::similar( a1, a2, 50 ) )		// I don't like this
    ...
    if( h1.bottom() <= h2.top() )
      ...
      if( lp.isflats() && !lp.ispit() ) { add_guess( 'B', 0 ); return; }
      ...
      if( charbox.bottom() > h2.vcenter() &&
        ...
        {
        if( b.top() >= charbox.top() && ( lp.ispit() || lp.isconvex() ) &&
            ( !rp.ispit() || h2.right() > h1.right() ) )
          add_guess( 'e', 1 );
        else if( b.top() >= charbox.top() &&
                 b.right() - rp[rp.pos(50)] > h1.right() && !rp.isctip() )
          add_guess( 'a', 1 );
