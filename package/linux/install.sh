...
mkdir -p /opt/local/osra/2.0.0 || { echo "Cannot create /opt/local/osra folder" 1>&2; exit; } 
mkdir -p /usr/local/bin || { echo "Cannot create /opt/local/osra folder" 1>&2; exit; }
cp --remove-destination package/* /opt/local/osra/2.0.0/ || { echo "Cannot copy to /opt/local/osra folder" 1>&2; exit; }
