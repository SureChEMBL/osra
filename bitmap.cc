...
Bitmap::Bitmap( const Bitmap & source, const Rectangle & re ) throw()
  ...
  for( int row = 0; row < height(); ++row )
    ...
    const std::vector< char > & datarow2 = source.data[row+tdiff];
...
void Bitmap::top( const int t ) throw()
  ...
  if( t == top() ) return;
  if( t < top() )
    data.insert( data.begin(), top() - t, std::vector< char >( width(), false ) );
  else
...
//
bool Bitmap::adjust_height() throw()
  {
  int row1, row2;
  ...
  if( row1 > top() || row2 < bottom() )
    {
    if( row1 <= row2 ) { top( row1 ); bottom( row2 ); }
    else return false;
    }
  return true;
  }
...
bool Bitmap::escape_left( int row, int col ) const throw()
  {
...
bool Bitmap::escape_top( int row, int col ) const throw()
  {
  if( get_bit( row, col ) ) return false;
  int l, r;
...
bool Bitmap::escape_right( int row, int col ) const throw()
  {
  if( get_bit( row, col ) ) return false;
  int u, d;
...
bool Bitmap::escape_bottom( int row, int col ) const throw()
  {
  if( get_bit( row, col ) ) return false;
  int l, r;
...
int Bitmap::follow_top( int row, int col ) const throw()
  {
...
int Bitmap::follow_bottom( int row, int col ) const throw()
  {
  if( !get_bit( row, col ) ) return row;
  std::vector< char > array;
  array.reserve( width() );
...
//
bool Bitmap::top_hook( int *hdiff ) const throw()
  {
...
//
bool Bitmap::bottom_hook( int *hdiff ) const throw()
  {
  int row, lcol = 0, rcol = 0, black_section = 0, wmax = 0;
